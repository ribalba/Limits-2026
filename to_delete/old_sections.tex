...
We implement impact disclosure at the HTTP server layer using an nginx module. The module first
loads a JSON registry that maps URI paths (or location blocks) to endpoint energy models (1). It then extracts runtime observables from nginx variables (request processing time, payload sizes, and optionally token count supplied by the application) (2). Subsequently, it evaluates the corresponding model to estimate energy per request and derives operational and embodied \(\mathrm{CO_2e}\) (3) and then injects response headers and optionally writes structured logs for downstream accounting (4)).

We used $ngx_http_js_module$ which uses a Java Script dialect called $njs$ to extend nginx. The selection was made since it requires no kernel patches and no cloud vendor energy APIs. We also implemented a helper script that automatically generates the json input for the nginx module. Thus, the data can be updated automatically.
...
A further modelling choice concerns whether per-request energy should be derived from idle-subtracted measurements or from total machine energy observed during the benchmark interval. Idle subtraction is appropriate when the goal is to estimate the marginal energy attributable to request handling, because it removes the baseline consumption that would occur even without the workload and therefore helps isolate the request-dependent signal. By contrast, for provider-side accounting or allocation, it can be reasonable to retain total energy or to combine marginal request energy with a separate baseline-allocation term, since real systems consume energy even at low utilisation. In this paper, we use total machine energy for model derivation, but treat this as an explicit accounting assumption rather than a neutral measurement detail. Any deployment using the method should therefore disclose whether reported request values are marginal, average, or include a share of baseline system energy.
...
\subsubsection{AI endpoint:} The AI endpoint is the clearest example of why application-supplied variables matter. If one looked only at the prompt-profile labels (\texttt{very-short}, \texttt{short}, \texttt{medium}, \texttt{long}, \texttt{very-long}), the measured runtimes are not strictly monotonic. In our run, the \texttt{medium} profile took longer than the \texttt{very-long} profile. On its face this could appear inconsistent with the expectation that longer prompts should consume more energy.
